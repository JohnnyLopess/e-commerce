...
function HomePage() {
  ...
  const renderProductCard = (product) => {
    ...
    return (
      <Link
        ...
        className="group bg-white rounded-2xl p-3 sm:p-4 flex-shrink-0 w-[160px] sm:w-44 md:w-48 relative border border-gray-100 hover:border-primary-200 hover:shadow-lg transition-all duration-300"
      >
        {hasDiscount && (
        ...
        <div className="relative mb-3">
          <picture>
            ...
            <img
              ...
              alt={product.name}
              className="w-full h-28 sm:h-32 md:h-36 object-contain group-hover:scale-105 transition-transform duration-300"
            />
          </picture>
          
          <button 
            className="absolute bottom-0 right-0 w-8 h-8 bg-primary-500 text-white rounded-full flex items-center justify-center opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 hover:bg-primary-600 active:scale-95"
            onClick={(e) => {
              e.preventDefault()
              // TODO: Add to cart
            }}
        ...
        <h3 className="font-medium text-[11px] sm:text-xs md:text-sm text-gray-700 line-clamp-2 mb-2 h-8 sm:h-9 md:h-10">
          {product.name}
        </h3>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {promoProducts.length > 0 && (
        <section id="ofertas" className="pb-8 sm:pb-10">
          <div className="max-w-7xl mx-auto px-2 sm:px-4">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center">
                  <i className="fa-solid fa-tags text-primary-500"></i>
                </div>
                <h2 className="text-lg sm:text-xl font-bold text-gray-800">Ofertas do dia</h2>
              </div>
              <Link to="/ofertas" className="text-primary-600 text-xs sm:text-sm font-medium hover:underline">
                Ver mais →
              </Link>
            </div>
      ...
      {collections.map((collection) => {
        ...
        return (
          <section key={collection.id} id={collection.slug} className="pb-8 sm:pb-10">
            <div className="max-w-7xl mx-auto px-2 sm:px-4">
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full ${color.split(' ')[0]} flex items-center justify-center`}>
                    <i className={`fa-solid ${icon} ${color.split(' ')[1]}`}></i>
                  </div>
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800">{collection.title}</h2>
                </div>
                <a href={`#${collection.slug}`} className="text-primary-600 text-xs sm:text-sm font-medium hover:underline">
                  Ver mais →
                </a>
              </div>
              <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-2 scrollbar-hide">
